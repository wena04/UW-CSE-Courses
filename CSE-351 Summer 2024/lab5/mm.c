...
void *mm_malloc(size_t size)
{
  ...
  // the found/created free space might be too big. better to dedicate exec amount of space.
  block_size = SIZE(ptr_free_block->size_and_tags);
  preceding_block_use_tag = ptr_free_block->size_and_tags & TAG_PRECEDING_USED;

  if (block_size - req_size < MIN_BLOCK_SIZE) // check if it is optimal to split found block into returning and unused parts
  {
    // mark current block as busy (with relevant prev block info), and mark this block busy in the next block
    (*ptr_free_block).size_and_tags = block_size | preceding_block_use_tag | TAG_USED;
    *((size_t *)UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE)) = (*ptr_free_block).size_and_tags;
    ((block_info *)UNSCALED_POINTER_ADD(ptr_free_block, block_size))->size_and_tags |= TAG_PRECEDING_USED;
  }
  else
  {
    // create new block
    ...
    *((size_t *)UNSCALED_POINTER_ADD(new_block, block_size - req_size - WORD_SIZE)) = (*new_block).size_and_tags;
    insert_free_block(new_block);

    // populate metadata for the current block
    (*ptr_free_block).size_and_tags = req_size | preceding_block_use_tag | TAG_USED;
    *((size_t *)UNSCALED_POINTER_ADD(ptr_free_block, req_size - WORD_SIZE)) = (*ptr_free_block).size_and_tags;
  }
...
void mm_free(void *ptr)
{
  ...
  // get this block and mark it as free
  block_info *block_to_free = (block_info *)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  size_t payload_size = SIZE(block_to_free->size_and_tags);
  ...
  *((size_t *)UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE)) = block_to_free->size_and_tags;
  ...
  // mark this block as free in the next block
  block_info *following_block = (block_info *)UNSCALED_POINTER_ADD(block_to_free, payload_size);
  if (following_block + MIN_BLOCK_SIZE < (block_info *)mem_heap_hi())
  {
    size_t following_block_payload_size = SIZE(following_block->size_and_tags);
    following_block->size_and_tags = following_block->size_and_tags & (~TAG_PRECEDING_USED);
    *((size_t *)UNSCALED_POINTER_ADD(following_block, following_block_payload_size - WORD_SIZE)) = following_block->size_and_tags;
  }
