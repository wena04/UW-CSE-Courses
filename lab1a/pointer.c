...
int int_size() {
  ...
  int* int_ptr2 = int_array+1;
  // Write code to compute size of an integer.
  return (int) ((unsigned char*)int_ptr2 - (unsigned char*)int_ptr1);
}
...
int double_size() {
  ...
  double* doub_ptr1 = doub_array;
  double* doub_ptr2 = doub_array+1;
  // Write code to compute size of a double.
  return (double) ((unsigned char*)doub_ptr2 - (unsigned char*)doub_ptr1);
}
...
int pointer_size() {
  double* ptr_array[10];
  double** ptr_ptr1 = ptr_array;
  double** ptr_ptr2 = ptr_array+1;
  // Write code to compute size of a pointer.
  return (int) ((unsigned char*)ptr_ptr2 - (unsigned char*)ptr_ptr1);
}
...
int within_array(int* int_array, int size, int* ptr) {
  // Your code here
  long int first_byte = (long int)int_array;
  long int last_byte = (long int)(int_array + size - 1);

  long int ptr_byte = (long int)ptr;

  /* shift*/
  int sign1 = (int)(ptr_byte - first_byte) >> 31;  // Sign of (ptr_byte - first_byte)
  int sign2 = (int)(last_byte - ptr_byte) >> 31;   // Sign of (last_byte - ptr_byte)

  /* If both signs are 0, then ptr is within the array boundaries */
