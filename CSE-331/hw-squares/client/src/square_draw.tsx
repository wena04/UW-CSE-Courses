...
export const SquareNodeElem = (props: SquareNodeProps): JSX.Element => {
  ...
  if (props.square.kind === "solid") {
    ...
    const left = props.width / 2n; // floored automatically
    const right = BigInt(Math.ceil(Number(props.width) / 2));
    const top = props.height / 2n;
    const bottom = BigInt(Math.ceil(Number(props.height) / 2));
    return (
      <td className={cls}>
        <table cellSpacing={0}>
        <tbody>
